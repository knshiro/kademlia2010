	...
	{    
		...
		if (file!=NULL)
		{
			...
			else
			{
				...
				char* hash_value;
				hash_value = json_object_get_string(json_object_object_get(json_object_object_get(message->header,"a"),"id"));
...
int kademHandleAnswerFindValue(struct kademMachine * machine, struct kademMessage * message, char *ip ,int port){
	...
	if(sent_query != NULL){
		...
		if ((find_query = find_key(machine->store_find_queries, sent_query_value) != NULL)){ 
			...
			kdm_debug("Find Value query not found\n");
		}
	}
	else
	{
			...
			if(FD_ISSET(machine->sock_local_rpc, &readfds)) {
				...
				if (strcmp(buffer,KADEM_QUERY) == 0){
					...
					if (strcmp(buffer,"print_routing_table") == 0){
						...
						kdm_debug("RECEIVED PUT FROM RPC");
